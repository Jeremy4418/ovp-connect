  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0faf5", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem 1rem", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ background: "white", borderRadius: "16px", border: "0.5px solid #e5e7eb", padding: "1.75rem", width: "100%", maxWidth: "440px" }}>
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
              <div><label style={lbl}>Ville *</label><input style={inp} placeholder="Tours" value={form.ville} onChange={e => set("ville", e.target.value)} /></div>
              <div><label style={lbl}>Code postal *</label><input style={inp} placeholder="37000" value={form.code_postal} onChange={e => set("code_postal", e.target.value)} /></div>
            </div>
            ...
            <div style={{ marginBottom: "10px" }}><label style={lbl}>État du bien *</label>
              <select style={inp} value={form.etat_bien} onChange={e => set("etat_bien", e.target.value)}>
                <option value="">Sélectionner...</option>
                <option value="neuf">Neuf</option><option value="ancien">Ancien</option><option value="les_deux">Les deux (peu importe)</option>
              </select>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
              <div><label style={lbl}>Surface (m²)</label><input style={inp} placeholder="80" type="number" value={form.surface} onChange={e => set("surface", e.target.value)} /></div>
              <div><label style={lbl}>Nb de pièces</label><input style={inp} placeholder="3" type="number" value={form.nb_pieces} onChange={e => set("nb_pieces", e.target.value)} /></div>
            </div>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <button style={{ ...btn, opacity: (!form.ville || !form.code_postal || !form.type_bien || !form.etat_bien || !form.budget) ? 0.4 : 1, flex: 2 }}
                disabled={!form.ville || !form.code_postal || !form.type_bien || !form.etat_bien || !form.budget}
        ...
        {step === 4 && (
          <div>
            ...
            <div style={{ marginBottom: "10px" }}><label style={lbl}>Revenus mensuels nets (€) *</label><input style={inp} placeholder="3000" type="number" value={form.revenus_mensuels} onChange={e => set("revenus_mensuels", e.target.value)} /></div>
            ...
            <div style={{ marginBottom: "1rem" }}><label style={lbl}>Message complémentaire (optionnel)</label><input style={inp} placeholder="Précisez votre projet..." value={form.message} onChange={e => set("message", e.target.value)} /></div>
            <div style={{ marginBottom: "10px", fontSize: "11px", color: "#9ca3af" }}>
              En soumettant ce formulaire, vous acceptez que vos données soient transmises à des professionnels de l'immobilier de votre secteur.
            </div>
            <div style={{ display: "flex", gap: "10px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "1.5rem", marginTop: "1.25rem" }}>
        {[{ icon: "🔒", text: "Données protégées" }, { icon: "⚡", text: "Réponse sous 24h" }, { icon: "✅", text: "Pros certifiés" }].map(t => (
