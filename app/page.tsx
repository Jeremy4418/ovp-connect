...
export default function Home() {
  ...
  const fetchCodePostal = async (ville: string) => {
    ...
    try {
      ...
      if (data[0]?.codesPostaux?.[0]) {
        set("code_postal", data[0].codesPostaux[0]);
      }
    } catch (e) {}
  };
  ...
  if (success) return (
    <div style={{ minHeight: "100vh", background: "#f0faf5", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      <div style={{ background: "white", borderRadius: "16px", border: "0.5px solid #e5e7eb", padding: "3rem 2rem", maxWidth: "400px", textAlign: "center" }}>
        ...
        <p style={{ fontSize: "14px", color: "#6b7280", lineHeight: 1.6 }}>Un professionnel de votre secteur va vous contacter très prochainement.</p>
      </div>
    </div>
  );
      ...
      <div style={{ background: "white", borderRadius: "16px", border: "0.5px solid #e5e7eb", padding: "1.75rem", width: "100%", maxWidth: "460px" }}>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
              <div>
                ...
                <input style={inp} placeholder="Tours" value={form.ville}
                  onChange={e => { set("ville", e.target.value); fetchCodePostal(e.target.value); }} />
              </div>
              <div>
                <label style={lbl}>Code postal</label>
                <input style={{ ...inp, background: "#f9fafb" }} placeholder="Auto" value={form.code_postal} onChange={e => set("code_postal", e.target.value)} />
              </div>
            </div>
        ...
        {step === 4 && (
          <div>
            ...
            <div style={{ marginBottom: "10px", fontSize: "11px", color: "#9ca3af" }}>En soumettant ce formulaire, vous acceptez que vos données soient transmises à des professionnels de l'immobilier de votre secteur.</div>
            <div style={{ display: "flex", gap: "10px" }}>
